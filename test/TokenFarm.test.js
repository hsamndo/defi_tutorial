...
contract('TokenFarm', ([owner, investor]) => {
    ...
    describe('Farming tokens', async () => {
        it('rewards investors for staking mDai tokens', async () => {
            ...
            assert.equal(result.toString(), tokens('100'), 'Mock DAI wallet balance correct before staking')
            ...
            result = await daiToken.balanceOf(investor)
            assert.equal(result.toString(), tokens('100'), 'investor Mock DAI wallet balance correct adter unstaking')

            result = await daiToken.balanceOf(tokenFarm.address)
            assert.equal(result.toString(), tokens('0'), 'Token Farm Mock DAI wallet balance correct adter unstaking')
